...
public class ItemServiceBackend extends GetPostChannelHandler {
        ...
        repo = new BatchedK2VCache<>(
                ...
                b -> b
                        ...
                        .removalListener((key, value, cause) -> {
                            ...
                            eventLoop.execute(() -> sha2id.remove(Hashing.sha256().hashBytes(value), key.intValue()));
                        })
        );
    }
    ...
    private ResponseFuture<S2CItemIdResponsePacket> pushItem(C2SPushItemRequest push) {
        eventLoop.assertThread();
        HashCode hashCode = SHA_256.hashBytes(push.itemStack());
        int id = sha2id.getInt(hashCode);
        if (id != -1) {
            var res = repo.get(id).orElse(null);
            if (res != null && Arrays.equals(res, push.itemStack())) {
                return new ResponseFuture<>(new S2CItemIdResponsePacket(id));
            } else {
                log.error("Duplicate SHA! old={}, new={}", Base64.getEncoder().encodeToString(res), Base64.getEncoder().encodeToString(push.itemStack()));
            }
        }
        id = ++lastId;
